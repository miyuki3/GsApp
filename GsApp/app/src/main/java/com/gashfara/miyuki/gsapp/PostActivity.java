...
public class PostActivity extends ActionBarActivity {
    ...
    public void onAttachFileButtonClicked(View v) {
        //ギャラリーを開くインテントを作成して起動する。アプリ間連携もintent。ギャラリーも一つのアプリ。
        Intent intent = new Intent();
        //フアイルのタイプを設定。画像だけをフィルタリングしている。
        intent.setType("image/*");
        //画像のインテント。相手のアプリに依存している。
    ...
    public void onAttachCameraFileButtonClicked(View v) {
        ...
        //設定を保存するパラメータを作成.ContentValuesをつかって値を渡すのはカメラの仕様。
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == IMAGE_CHOOSER_RESULTCODE) {
            ...
            Uri result;
            if(data != null) {
                result = data.getData();//dataに入っているのはギャラリーの画像。
            }else {
                result = mImageUri;//カメラで画像を取ったときmImageUriにはいっている。
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            ...
            try {
                ...
                int bmp_height= bmp.getHeight();
                    ...
                    Log.d("haba", (String.valueOf(bmp_width)));
                    Log.d("takasa", (String.valueOf(bmp_height)));

                float bmp_per = (float) 100 / bmp_width ;

                    Log.d("per", (String.valueOf(bmp_per)));

                bmp_height *= bmp_per ;

                    Log.d("takasa", (String.valueOf(bmp_height)));

                Bitmap bmp2= Bitmap.createScaledBitmap(bmp, 100, bmp_height, false);

//マトリックスでリサイズする方法（0,5Fのスケールをもっと小さくして試してみる）http://qiita.com/gonabe/items/32e47d2827b3ec808bd4
